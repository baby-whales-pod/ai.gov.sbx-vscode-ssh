...
# Fetch ANTHROPIC_API_KEY from 1Password when not already in env. This is what
# makes the .app launcher work: a Finder-double-clicked .app inherits launchd's
# minimal env, not the user's shell env, so ANTHROPIC_API_KEY is missing —
# without it, sbx skips writing `apiKeyHelper` to the sandbox's Claude
# settings.json and Claude prompts for /login. Keeping the fetch in-script
# (vs `launchctl setenv` from .zshrc) confines the key to this process subtree
# rather than exposing it to every GUI process under launchd.
if [ -z "${ANTHROPIC_API_KEY:-}" ] && command -v op >/dev/null 2>&1; then
  OP_REF="${OP_ANTHROPIC_REF:-op://dev/anthropic/credential}"
  if key="$(op read "$OP_REF" 2>/dev/null)" && [ -n "$key" ]; then
    export ANTHROPIC_API_KEY="$key"
  else
    echo "Warning: 'op read $OP_REF' failed — is 1Password Desktop running and unlocked?" >&2
  fi
  unset key
fi
